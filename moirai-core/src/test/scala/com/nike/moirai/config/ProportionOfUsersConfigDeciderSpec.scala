...
class ProportionOfUsersConfigDeciderSpec extends FunSpec with Matchers with GeneratorDrivenPropertyChecks {
  describe("A basic implementation of ProportionOfUsersConfigDecider") {
    val configReader: ConfigReader[Map[String, Double]] = new ConfigReader[Map[String, Double]] {
      override def enabledProportion(config: Map[String, Double], featureIdentifier: String): Optional[java.lang.Double] =
        config.get(featureIdentifier).map(Double.box).asJava
      override def featureGroup(config: Map[String, Double], featureIdentifier: String): Optional[String] = Optional.empty()
      override def enabledUsers(config: Map[String, Double], featureIdentifier: String): java.util.Collection[String] = ???
      override def featureEnabled(config: Map[String, Double], featureIdentifier: String): Optional[java.lang.Boolean] = ???
    }

    val decider = ConfigDeciders.proportionOfUsers(configReader)
    ...
    describe("featureGroup") {
      val configReader: ConfigReader[Map[String, Double]] = new ConfigReader[Map[String, Double]] {
        override def featureGroup(config: Map[String, Double], featureIdentifier: String): Optional[String] = featureIdentifier match {
